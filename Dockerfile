...
# ============ Stage 1: 编译前端 ============
FROM node:22.20-alpine AS frontend
...
RUN cd backend && GOPROXY='https://goproxy.cn,direct' CGO_ENABLED=0 go build -ldflags "-w -s -X 'WorkReport/common.BuildTime=$(date "+%F %T")' -X 'WorkReport/common.CommitID=$(git rev-parse HEAD 2>/dev/null || echo unknown)' -X 'WorkReport/common.GoVersion=$(go version)' -X 'WorkReport/common.Version=$(git describe --tags 2>/dev/null || echo unknown)' -X 'WorkReport/common.BuildUser=docker'" -o /app/WorkReport . && cd /app && upx /app/WorkReport
